 ...
 */
function computeFinishedV1(
  job: Pick<Job, 'parts' | 'boards' | 'settings'>,
  clearanceOf: ReadonlyMap<string, Partial<Record<Axis, number>>>,
): Map<string, FinishedDims> {
  ...
  function finishedOf(d: DimRef): Value {
    ...
    if (result.ok) {
      ...
      const clearance = clearanceOf.get(part.id)?.[d.axis] ?? 0
...
/** 以前の版で出ていた、部材ごとの寸法 */
export interface V1Dimensions {
  /** 仕上がり寸法（式の計算結果 − 逃げ）。エラーがあれば null */
  finished: Record<Axis, number> | null
  /** 厚みの寸法（手で選んだ軸、または自動判定の軸）。決まらなければ null */
...
/**
 * 以前の版の計算で、部材ごとの仕上がり寸法・厚みの寸法・木取り寸法を求める。キーは部材の id。
 * clearanceOf は部材の id → その部材の逃げ（mm）
 */
export function computeV1Dimensions(
  job: Pick<Job, 'parts' | 'boards' | 'settings'>,
  clearanceOf: ReadonlyMap<string, Partial<Record<Axis, number>>>,
): Map<string, V1Dimensions> {
  const fin = computeFinishedV1(job, clearanceOf)
  const boardById = new Map(job.boards.map((b) => [b.id, b]))
  const out = new Map<string, V1Dimensions>()
  for (const p of job.parts) {
